...
deputes_bd=[]
deputes_age=[]
deputes_parti_national=[]
deputes_parti_europeen=[]
...
for depute_parti_europeen in soup.find_all("politicalGroup"):
    deputes_parti_europeen.append(depute_parti_europeen.get_text())

for depute_parti_national in soup.find_all("nationalPoliticalGroup"):
    deputes_parti_national.append(depute_parti_national.get_text())
...
for i in range(0,len(deputes_ids)):
    ...
    DDN=deputes_bd[i]
    Age=deputes_age[i]
    parti_europeen=deputes_parti_europeen[i]
    parti_national=deputes_parti_national[i]
    sql = "INSERT INTO Deputes (ID, FullName, DDN, Age,parti_europeen, parti_national) VALUES (?, ?, ?, ?, ?, ?)"
    val = (ID, FullName, DDN, Age, parti_europeen, parti_national)
    cur.execute(sql, val)
con.commit()
con.close()  
...
recapitulatif=[deputes_ids,deputes_names,deputes_bd,deputes_age,deputes_parti_europeen,deputes_parti_national]
